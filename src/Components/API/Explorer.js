...
export function getSecsAgo(h) {
  const ago = parseInt(h, 10);
  const now = new Date();
  const d = now - ago;
...
export function abbreviateValidator(s) {
  const len = s.length;
  return s.slice(0, 10) + "..." + s.slice(len - 6);
}

//get the latest n elements in an array
export function updateBlockArray(arr, elem, len) {
...
//get the latest n elements in an array
export function updateTxnArray(arr, elems, len) {
...
export async function getTxnByHash(url, txnHash) {
  let quorumTxn = {
    blockHash: "error",
    blockNumber: -1,
    from: "",
...
export async function getBlockByNumber(url, blockNumber) {
  let quorumBlock = {
    statusText: "error",
    number: -1,
    hash: "",
